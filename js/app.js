...
(function () {
    ...
    function renderBar() {
        ...
        if (filtered.length === 0) { barEmpty.style.display = 'block'; }
        else {
    ...
    function updateFooter(count) {
        var t = count || state.memos.length;
        var c = state.memos.filter(function (x) { return x.is_completed; }).length;
    ...
    function toggle() {
        ...
        api('toggle', { id: id }).then(function (j) {
            ...
            btnToggle.textContent = state.memos[i].is_completed ? '恢复' : '标记完成';
    ...
    // Clear format
    $('btn-clear-fmt').addEventListener('mousedown', function (e) {
        e.preventDefault();
        document.execCommand('removeFormat', false, null);
        eContent.focus();
    });

    // Toggle format button active states when selection changes
    ...
    document.addEventListener('selectionchange', function () {
        if (document.activeElement === eContent) toggleFormatBtns();
    });
