    ...
    <div class="auth-box">
        ...
        <form method="POST">
            ...
            <div class="form-row" style="gap:16px">
                <div class="form-field"><label for="username">用户名</label><input type="text" id="username" name="username" class="input<?php echo isset($errors['username'])?' input-error':''; ?>" required maxlength="50" placeholder="3-50位" value="<?php echo htmlspecialchars($old['username']??'', ENT_QUOTES,'UTF-8'); ?>"><?php if(!empty($errors['username'])) echo '<span class="field-error">'.htmlspecialchars($errors['username'], ENT_QUOTES,'UTF-8').'</span>'; ?></div>
                <div class="form-field"><label for="email">邮箱</label><input type="email" id="email" name="email" class="input<?php echo isset($errors['email'])?' input-error':''; ?>" required placeholder="your@email.com" value="<?php echo htmlspecialchars($old['email']??'', ENT_QUOTES,'UTF-8'); ?>"><?php if(!empty($errors['email'])) echo '<span class="field-error">'.htmlspecialchars($errors['email'], ENT_QUOTES,'UTF-8').'</span>'; ?></div>
                <div class="form-field"><label for="password">密码</label><input type="password" id="password" name="password" class="input<?php echo isset($errors['password'])?' input-error':''; ?>" required minlength="6" placeholder="至少6位"><?php if(!empty($errors['password'])) echo '<span class="field-error">'.htmlspecialchars($errors['password'], ENT_QUOTES,'UTF-8').'</span>'; ?></div>
                <div class="form-field"><label for="confirm_password">确认密码</label><input type="password" id="confirm_password" name="confirm_password" class="input<?php echo isset($errors['confirm_password'])?' input-error':''; ?>" required minlength="6" placeholder="再次输入"><?php if(!empty($errors['confirm_password'])) echo '<span class="field-error">'.htmlspecialchars($errors['confirm_password'], ENT_QUOTES,'UTF-8').'</span>'; ?></div>
